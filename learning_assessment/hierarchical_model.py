...
class ObjectiveModels:
    ...
    def build(
        ...
        for objective in self.objectives:
            coords[f"assignment_{objective}"] = self.assignments[objective]

        print("Building model")
        with pm.Model(coords=coords) as self.model:
            # gp hyperparameters
            alpha = pm.Gamma("alpha", alpha=2.0, beta=1.0)
            beta = pm.Gamma("beta", alpha=2.0, beta=1.0)
            sigma = pm.HalfNormal("sigma", sigma=1.0)

            # Loop over learning objectives
            for objective in self.objectives:
                # Drop zeros for sampling (missing assessments)
                ...
                # gp parameters
                ell = pm.Beta(f"ell_{objective}", alpha=alpha, beta=beta)
                eta = pm.HalfNormal(f"eta_{objective}", sigma=sigma)
                ...
                f = self.gps[objective].prior(
                    ...
                    dims=["student", f"assignment_{objective}"],
                )
        ...
        # Sample prior
        with self.model:
            self.model.prior = pm.sample_prior_predictive()
    ...
    def sample(self, thin_predictive=10):
        ...
        with self.model:
            ...
            for objective in self.objectives:
                ...
                _ = pm.Binomial(
                    f"score_pred_{objective}",
                    n=int(self.max_score) - 1,  # offset for missing assignments
                    p=p_pred,
                    dims=["student", "norm_date"],
                )
                var_names += [f"p_pred_{objective}", f"score_pred_{objective}"]
            self.model.posterior_predictive = az.extract(
                pm.sample_posterior_predictive(
                    self.model.trace.sel(draw=slice(None, None, thin_predictive)),
                    var_names=var_names,
    ...
    def student_summary(self, percentiles=[5.0, 95.0]):
        ...
        for i, student in enumerate(self.students):
            ...
            for objective in self.objectives:
                ...
                self.mastery[student][objective] = p_mastery > self.mastery_threshold
                ...
                fig.update_layout(
                    title=dict(
                        text=(
                            ...
                            + f"Predictive Probability on {self.data["Date"].max()}"
                        )
                    ),
                    xaxis=dict(
                        title=dict(text="Score"),
            ...
            fig = go.Figure()
            for objective in self.objectives:
                if self.mastery[student][objective][-1] > self.mastery_threshold:
                    fig.add_trace(
                        go.Histogram(
                            x=[1.0],
                            name=f"{objective}",
...
def main():
    ...
    PARSER.add_argument(
        "--intercept", type=int, default=0.0, help="Gaussian process mean"
    )
    PARSER.add_argument(
        "--prior_ell",
        type=float,
